...
fifa.df.sub$Calculated_Age <- fifa.df.sub %>% 
...
```{r}
fifa.df.sub$fifa.df.sub[1:10, c("Name", "Birth_Date", "Age", "Calculated_Age")]
Age <- ifelse((fifa.df.sub$Calculated_Age!=fifa.df.sub$Age) & (!is.na(fifa.df.sub$Calculated_Age)), fifa.df.sub$Calculated_Age, fifa.df.sub$Age)
```
