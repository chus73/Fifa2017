...
Además, tenemos que tener especial cuidado debido a que las medidas en metros, no cumplen con una homegeneidad de formato, como se puede ver en la siguiente selección de registros.
...
- La longitud de las flechas indican la importancia que tienen las variables originales en las componentes.En todos los gráficos vemos que la longitud es similar, [no destacando ninguna variable sobre el resto]{.ul}.
...
```
Visualmente podemo ver como la pendiente de la recta se hace más acusada a partir de la segunda componente, quedando PC3 y PC4 como mínimos de la curva.

*Agradecimientos a: https://datavizpyr.com/how-to-make-scree-plot-in-r-with-ggplot2/ por esta última parte.*


# Archivo final

En este trabajo se ha utilizado con un subset de datos del archivo original que ha sido analizado y tratado. A continuación realizaremos la escritura del archivo en formato csv.

```{r}
file_name <- "fifa_clean.csv"
write.csv(fifa.df.sub, file = file_name, sep=",", dec=".")
```
Este trabajo está compuesto de los siguientes ficheros:

- Fifa2017.Rmd con éste trabajo en Markdown y R.
- Fifa2017.html, traducción del trabajo en formato html.
- Fifa2017.pdf, fichero de texto pdf.
- Fifa_clean.csv, fichero con los datos procesados.
