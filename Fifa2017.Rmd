...
La variable **Contract_Expiry**, aunque se encuentra en formato numérico, no debemos olvidar que corresponde a un año y por tanto, su naturaleza es ser parte de una fecha. Por otro lado, es una variable que cuenta con un valor desconocido, que nos obliga a tratarlo para poder realizar el estudio. Podríamos optar por eliminar el registro, o no tenerlo en cuenta para este estudio dado que corresponde a una sola observación entre 17588, pero estaríamos perdiendo información relevante de este jugador. Por ello, considero más práctico inferir la media de la población, asumiendo el riesgo de introducir un dato por criterios estadísticos, que no se corresponderá con la realidad.
...
quantitative_study <- function(df, col_names) {
  ...
  winsor_media <- as.data.frame(lapply(df[, col_names], winsor.mean)) %>% mutate(estimator=c("winsor_media"))
...
Por último, encontramos un punto residual fuera de la nube de puntos, que se repite en los tres gráficos de componentes. Este punto, podría ser clasificado como outlier y sacado fuera del modelo de datos. Vamos a ver los valores de las variables para este punto.
